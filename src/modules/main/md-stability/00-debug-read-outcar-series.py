...
def _parse_single_outcar(
    ...
    if not pos_force_tables:
        LOGGER.warning("  No POSITION/TOTAL-FORCE blocks found")
        return [], [], [], []
...
if __name__ == "__main__":
    ...
    LOGGER.info(f"Frames: {len(traj.frames)}")
    LOGGER.info(f"Energies: {traj.energies}")
    LOGGER.info(f"Temperatures: {traj.temperatures}")
    LOGGER.info(f"Forces shape: {traj.forces_strided[0].shape}")
    LOGGER.info(f"Times (ps): {traj.times_strided_ps}")
