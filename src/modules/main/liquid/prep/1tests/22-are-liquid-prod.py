...
def _analyze_single_poscar(poscar_path: Path, bin_width: float = 0.1) -> RDFResult:
    """
    Worker function: compute RDF and classify (no I/O).
    ...
    Returns:
        RDFResult with analysis results or error information
    """
    try:
        r, g_r = calculate_rdf(poscar_path, save_plot=False, bin_width=bin_width)

        flag, votes, metrics = classify_solid_like_simple(
            r,
            g_r,
            bin_width=bin_width,
            smooth_win=5,
...
def batch_rdf_analysis(
    ...
    # Parallel processing
    worker = partial(_analyze_single_poscar, bin_width=bin_width)
...
if __name__ == "__main__":
    ...
    if not poscar_paths:
        ...
        batch_rdf_analysis(
            poscar_paths, output_dir=base_dir, bin_width=0.1, max_workers=8
        )
