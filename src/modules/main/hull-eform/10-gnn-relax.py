...
METHOD_NAME = Mace_mpa_0.__name__
FMAX = 0.02
...
def main():
    ...
    try:
        ...
        if OUTPUT_DIR:
            ...
            # Save CSVs
            vasp_csv = OUTPUT_DIR / "vasp_eos.csv"
            vasp_df.to_csv(vasp_csv, index=False)
            LOGGER.info(f"\n  ✓ Saved VASP EOS: {vasp_csv}")

            if len(gnn_df) > 0:
                gnn_csv = OUTPUT_DIR / f"gnn_{METHOD_NAME}_eos.csv"
                ...
                try:
                    ...
                    comparison_csv = OUTPUT_DIR / f"comparison_{METHOD_NAME}.csv"
