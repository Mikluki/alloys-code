...
def _extract_binary_heatmap_data(
...
def _render_binary_heatmap_plotr(
    matrix: np.ndarray,
    elements: List[str],
    title: str,
...
def plot_formation_energy_heatmap(
    ...
    matrix, elements = _extract_binary_heatmap_data(structures, energy_source)

    if matrix.size == 0:
        raise ValueError("No binary alloy data found for heatmap")

    return _render_binary_heatmap_plotr(
        matrix,
        elements,
...
def plot_delta_heatmap(
    ...
    matrix, elements = _extract_binary_heatmap_data(preprocessed, energy_source)
